...
For a bounded external question, call a foreground `scout` through the installed pi-subagents `Agent` tool. For multiple independent source questions, issue all calls together with `run_in_background: true` and let smart join return the group:
...
For multiple independent questions, add `run_in_background: true` to each resolved call and issue them together. The parent checks citations, resolves source conflicts, and owns the recommendation. Do not use Fabric agent orchestration.
