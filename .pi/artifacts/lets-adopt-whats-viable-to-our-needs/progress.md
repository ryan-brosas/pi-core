...
## Research: Conversation-aware `/research` → `/create` handoff (2026-07-25)
...
- Note: this report was appended here because `.pi/artifacts/.active` currently resolves to this slug. That behavior itself exposes a cross-task contamination risk when the active feature is unrelated to the research topic.
...
- **Artifact routing can attach research to the wrong feature.** `/research` appends to any existing `.active` slug without validating topic relevance; this investigation was therefore persisted under an unrelated active feature. Source: `.pi/prompts/research.md:64,145`.
...
5. Make `/research` end with the same compact handoff shape. Append it only when the active slug is demonstrably related; otherwise leave it in the conversation for immediate `/create` and clearly report that it is not durable.
...
- Important: full upstream MIT notice was missing — confirmed and fixed from pinned commit `d1993d3`.
